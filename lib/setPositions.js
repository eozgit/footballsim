...
function setKickOffTeamGoalScored(matchDetails) {
  ...
  let thisIndex = matchDetails.kickOffTeam.players.findIndex(
    (thisPlayer) => thisPlayer.name == scorer,
  );
...
function setSecondTeamGoalScored(matchDetails) {
  ...
  let thisIndex = matchDetails.secondTeam.players.findIndex(
    (thisPlayer) => thisPlayer.name == scorer,
  );
...
function setIntentPosition(matchDetails, closestPlayer) {
  ...
  let kickOffTeamCheck = kickOffTeam.players.find(
    (thisPlayer) => thisPlayer.playerID == ball.Player,
  );
  let secondTeamCheck = secondTeam.players.find(
    (thisPlayer) => thisPlayer.playerID == ball.Player,
  );
  let kickTeam = kickOffTeamCheck
    ? kickOffTeam
    : secondTeamCheck
      ? secondTeam
      : 'none';
...
function setLooseintentPOS(matchDetails, thisTeam, closestPlayer) {
  ...
  for (let player of thisTeam.players) {
    ...
    if (player.playerID == closestPlayer.playerID)
      ...
      let southwards = ['south', 'southwest', 'southeast'].includes(
        ball.direction,
      );
      let northwards = ['north', 'northwest', 'northeast'].includes(
        ball.direction,
      );
      let newYPOS;
