...
function setTopFreekick(matchDetails) {
  ...
  let halfwayToLastQtr = common.isBetween(
    ballY,
    pitchHeight / 2,
    pitchHeight - pitchHeight / 4,
  );
  let upperFinalQtr = common.isBetween(
    ballY,
    pitchHeight - pitchHeight / 4,
    pitchHeight - pitchHeight / 6 - 5,
  );
  let lowerFinalQtr = common.isBetween(
    ballY,
    pitchHeight - pitchHeight / 6 - 5,
    pitchHeight,
  );
...
function setBottomFreekick(matchDetails) {
  ...
  let defence =
    kickOffTeam.players[0].originPOS[1] > pitchHeight / 2
      ? secondTeam
      : kickOffTeam;
  let hundredToHalfway = common.isBetween(
    ballY,
    pitchHeight / 2 - 1,
    pitchHeight - 100,
  );
...
function setTopOneHundredYPos(matchDetails, attack, defence) {
  ...
  for (let player of attack.players) {
    if (player.position == 'GK')
      player.currentPOS = matchDetails.ball.position.map((x) => x);
    if (player.position != 'GK')
...
function setTopOneHundredToHalfwayYPos(matchDetails, attack, defence) {
  let { ball } = matchDetails;
  let [, pitchHeight] = matchDetails.pitchSize;
  let goalieToKick = common.isBetween(
    ball.position[1],
    0,
    pitchHeight * 0.25 + 1,
  );
  ...
  for (let player of attack.players) {
    if (kickPlayer.position == 'GK') {
      if (player.position == 'GK')
        player.currentPOS = ball.position.map((x) => x);
      if (player.name != kickPlayer.name) {
        let newYPOS = common.upToMax(
  ...
  for (let player of defence.players) {
    if (kickPlayer.position == 'GK') {
      ...
      player.currentPOS = [
        player.originPOS[0],
        parseInt(pitchHeight * 0.75 + 5, 10),
      ];
    } else {
      player.currentPOS = [
        player.originPOS[0],
...
function setBottomOneHundredToHalfwayYPos(matchDetails, attack, defence) {
  ...
  for (let player of defence.players) {
    if (kickPlayer.position == 'GK') {
      ...
      player.currentPOS = [
        player.originPOS[0],
        parseInt(pitchHeight * 0.25 - 5, 10),
      ];
    } else {
      player.currentPOS = [
        player.originPOS[0],
...
function setTopHalfwayToBottomQtrYPos(matchDetails, attack, defence) {
  ...
  ball.Player = kickPlayer.playerID;
  ball.withTeam = attack.teamID;
  let ballInCentre = common.isBetween(
    ball.position[0],
    pitchWidth / 4 + 5,
  ...
  for (let player of attack.players) {
    if (player.position == 'GK')
      ...
      let maxYPOSCheck = common.upToMax(
...
function setBottomHalfwayToTopQtrYPos(matchDetails, attack, defence) {
  ...
  for (let player of attack.players) {
    if (player.position == 'GK')
      ...
      let maxYPOSCheck = common.upToMin(
        ball.position[1] - common.getRandomNumber(150, 300),
        pitchHeight * 0.25,
      );
      if (player.name != kickPlayer.name)
...
function setTopBottomQtrCentreYPos(matchDetails, attack, defence) {
  ...
  for (let player of defence.players) {
    ...
    if (player.position == 'GK')
      player.currentPOS = player.currentPOS = player.originPOS.map((x) => x);
    else if (['CB', 'LB', 'RB'].includes(player.position)) {
      player.currentPOS = [
...
function setTopLowerFinalQtrBylinePos(matchDetails, attack, defence) {
  ...
  for (let player of defence.players) {
    ...
    if (position == 'GK')
      player.currentPOS = player.currentPOS = originPOS.map((x) => x);
    else if (['CB', 'LB', 'RB'].includes(position)) {
