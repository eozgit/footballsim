...
function decideMovement(closestPlayer, team, opp, matchDetails) {
  ...
  for (const thisPlayer of team.players) {
    ...
    if (thisPlayer.currentPOS[0] != 'NP') {
      ...
      let possibleActions;
      possibleActions = actions.findPossActions(
        thisPlayer,
        team,
        opp,
        ballToPlayerX,
...
function handleBallPlayerActions(matchDetails, thisPlayer, team, opp, action) {
  ...
  if (ballActions.includes(action)) {
    ...
    if (action === `cleared` || action === `boot`) {
      ...
      let newPosition = ballMovement.ballCrossed(
        matchDetails,
        team,
        thisPlayer,
      );
      ...
      let newPosition = ballMovement.throughBall(
        matchDetails,
        team,
        thisPlayer,
      );
      ...
      let newPosition = ballMovement.penaltyTaken(
        matchDetails,
        team,
        thisPlayer,
      );
...
function getInterceptMovement(player, opposition, ballPosition, pitchSize) {
  let move = [0, 0];
  let intcptPos = getInterceptPosition(
    player.currentPOS,
    opposition,
    ballPosition,
    pitchSize,
...
function getRunMovement(matchDetails, player, ballX, ballY) {
  ...
  let formationDirection = setPositions.formationCheck(
    player.intentPOS,
    player.currentPOS,
  );
...
function team1atBottom(team1, team2, pitchHeight) {
  ...
  let topPlayerOffsidePosition = common.isBetween(
  ...
  let btmPlayerOffsidePosition = common.isBetween(
