...
function shotMade(matchDetails, team, player) {
  ...
  let endPos = calcBallMovementOverTime(
    matchDetails,
    player.skill.strength,
    shotPosition,
    player,
...
function checkGoalScored(matchDetails) {
  let { ball, half, kickOffTeam, secondTeam } = matchDetails;
  ...
  let nearKOGoalieX = common.isBetween(
    ballX,
    KOGoalie.currentPOS[0] - ballProx,
    KOGoalie.currentPOS[0] + ballProx,
  );
  let nearKOGoalieY = common.isBetween(
    ballY,
    KOGoalie.currentPOS[1] - ballProx,
    KOGoalie.currentPOS[1] + ballProx,
  );
  let nearSTGoalieX = common.isBetween(
    ballX,
    STGoalie.currentPOS[0] - ballProx,
    STGoalie.currentPOS[0] + ballProx,
  );
  let nearSTGoalieY = common.isBetween(
    ballY,
...
function throughBall(matchDetails, team, player) {
  ...
  let closePlyPos = [0, 0];
  let playersInDistance = getPlayersInDistance(
    team,
    player,
