...
function runTest() {
  mocha.describe('bottomTeamPlayerHasBallInMiddle()', function () {
    mocha.it('In middle of pitch, no opp. near, shooting over 85', async () => {
      ...
      let parameters = actions.bottomTeamPlayerHasBallInMiddle(
        playerInformation,
        player.position,
        player.skill
      );
  ...
  mocha.describe('topTeamPlayerHasBallInBottomPenaltyBox()', function () {
    mocha.it(
      'Close, no opposition near, no team mate close, further than shooting skills',
      async () => {
        let matchDetails = await common.readFile(
        ...
        let team = matchDetails.secondTeam;
        let opposition = matchDetails.kickOffTeam;
        let parameters = actions.topTeamPlayerHasBallInBottomPenaltyBox(
          matchDetails,
          player,
          team,
          opposition
    ...
    mocha.it(
      'Close, opp. near not ahead, no tmate close, further than shooting skills',
      async () => {
        let matchDetails = await common.readFile(
          'test/input/actionInputs/topTeamHasBallInBottomPenaltyBox.json'
        );
    ...
    mocha.it(
      'Close, no opposition ahead, no tmate close, further than shooting skills',
      async () => {
        let matchDetails = await common.readFile(
          'test/input/actionInputs/topTeamHasBallInBottomPenaltyBox.json'
        );
